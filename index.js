...
function printList() {
  ...
  for (var user in userList) {
    if (userList.hasOwnProperty(user)) {
      table.push([user, userList[user].url, userList[user].keyCount, userList[user].keyTypes, userList[user].keyBits]);
    }
  }
  ...
  for (var user in userList) {
    if (userList.hasOwnProperty(user)) {
      github.user.getKeysFromUser({
        ...
        if (keyCount == 0) {
          userList[this.user].keyCount = chalk.bgRed.bold(keyCount);
        } else {
...
function listOrgs() {
  github.user.getOrgs({
    ...
    console.log("Org Memberships (audit an org using the '-o <organization name>' parameter)");
